...
// Global texture
L_texture g_texture;
std::string g_texture_path{"../assets/image/foo-sprites.png"};
...
auto main(int argc, char* args[]) -> int {
    ...
    if (!init()) {
        ...
        if (!load_image()) {
            ...
            /* before entering the main loop, we declare a variable to keep track of the
            ...
            while (quit == false) {
                ...
                constexpr int animation_frames_per_sprite = 6;
                if (frame / animation_frames_per_sprite >= animation_frames) {
                    frame = 0;
                }

                /* after the event loop, we update the animation logic
                 * we increment the frame counter every frame, but we do not want to change
                 * the sprites every frame, or it will be too fast
                 * our app runs at 60fps but our animation runs at 10fps,
                 * to convert we divide the frame counter by 6
                 * we also want the animation to loop, so when it goes past the 4th frame we reset
                 */

                // Set sprite clips
                constexpr float sprite_width = 64;
                constexpr float sprite_height = 205;
                SDL_FRect sprite_clips[animation_frames] = {
                    {sprite_width * 0, 0.F, sprite_width, sprite_height},
                    {sprite_width * 1, 0.F, sprite_width, sprite_height},
                    {sprite_width * 2, 0.F, sprite_width, sprite_height},
                    {sprite_width * 3, 0.F, sprite_width, sprite_height}
                };

                /* we define the clip rectangles, they all have the same width/height and y position
                 * we only need to adjust the x position to get the next one
                 */
                ...
                SDL_FRect* current_clip = &sprite_clips[frame / animation_frames_per_sprite];
                g_texture.render(
                    (g_screen_width - sprite_width) * 0.5F,
                    (g_screen_height - sprite_height) * 0.5F, current_clip
                );

                /* in our rendering we get the current sprite clip and render it to the screen
                 * this will cycle through our sprites
                 */
...
/* Addendum
 * Animation is a rabbit hole
 * Completely ignoring the artistic side of it, animation is a very large field, there are concepts
 * like skeletal animation, animation blending, morphing, animation channels, and more that are all
 * out of the scope of this tutorial but are still key to AAA game engines.
 * what is common to animation is that they are all sequences of values, here we use a simple
 * array of SDL_FRects to represent our sequence of values, but in a real engine you would most
 * likely have an animation class with a get_current_sprite(float current_time) method or similar.
 * with animation encapsulated in a class you can very easily do things like pause, change the
 * animation speed, reverse it, looping, etc. which are common animation operations
 */
