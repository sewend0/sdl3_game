...
// Level dimensions
constexpr int g_level_width{1280};
constexpr int g_level_height{960};

/* we will be moving around a space larger than the screen,
 * so we need to define those 'level' dimensions
 */
...
public:
    ...
    auto move() -> void;
    auto render(SDL_FRect camera) -> void;

    // Position accessors
    auto get_pos_x() -> int;
    auto get_pos_y() -> int;

    /* our Dot class now takes a rect/camera for rendering,
     * and it has accessor functions for the dot's position
     */
...
std::string g_texture2_path{"../assets/image/background-2.png"};
...
auto Dot::move() -> void {
    pos_x += vel_x;
    if (pos_x < 0 || pos_x + dot_width > g_level_width) {
        pos_x -= vel_x;
    }

    pos_y += vel_y;
    if (pos_y < 0 || pos_y + dot_height > g_level_height) {
        pos_y -= vel_y;
    }
}

auto Dot::render(SDL_FRect camera) -> void {
    g_texture1.render(static_cast<float>(pos_x) - camera.x, static_cast<float>(pos_y) - camera.y);
}

auto Dot::get_pos_x() -> int {
    return pos_x;
}

auto Dot::get_pos_y() -> int {
    return pos_y;
}

/* instead of bounding the motion within the screen, we use the dimensions of the level now
 * we now render the dot relative to the position of the camera
 * this is done by subtracting the position of the dot by the position of the camera
 * lastly, we have our complete position accessors
 */
...
auto main(int argc, char* args[]) -> int {
    ...
    if (!init()) {
        ...
        if (!load_media()) {
            ...
            // Defines the camera area
            SDL_FRect camera{0.F, 0.F, g_screen_width, g_screen_height};

            /* before going into the main loop
             * we declare an SDL_FRect to define our camera
             */
            while (quit == false) {
                cap_timer.start();
                ...
                camera.x = dot.get_pos_x() + Dot::dot_width * 0.5F - g_screen_width * 0.5F;
                camera.y = dot.get_pos_y() + Dot::dot_height * 0.5F - g_screen_height * 0.5F;

                // Bound the camera
                if (camera.x < 0) {
                    camera.x = 0;
                } else if (camera.x + camera.w > g_level_width) {
                    camera.x = g_level_width - camera.w;
                }
                if (camera.y < 0) {
                    camera.y = 0;
                } else if (camera.y + camera.h > g_level_height) {
                    camera.y = g_level_height - camera.h;
                }

                /* when the dot moves, we want to center our camera over it
                 * we also don't want the camera to show anything that is outside the level
                 * so after it moves we keep it within the bounds of the level
                 */
                ...
                // Show background
                g_texture2.render(0.F, 0.F, &camera);

                // Render dot
                dot.render(camera);

                /* we only want to render the part of the background shown by the camera,
                 * and we show the dot relative to the camera
                 */
...
/* Addendum
 * Typical camera functionality
 * most game engines will make sure objects are first in the cameras field of view before rendering
 * them, this is an especially useful optimization within 3D engines and still very useful in 2D.
 * a simple way of doing this is by checking if the object collides with the AABB of the 2D camera
 * this does have some overhead, but it is quickly worth it if you have a lot of objects
 * we only used an SDL_FRect for our camera but if our game was any more complicated it would
 * warrant creating a separate camera class to keep the camera data and the code that moves the
 * camera together. camera behavior can get very complex with animations, tracking logic, etc.
 *
 * Decoupling rendering data from rendering
 * as previously mentioned, it is considered good practice to separate the data of what needs to
 * be rendered from the code the actually renders it
 * right now we only have two objects that need to be rendered to the camera, the dot and screen.
 * however, when you have dozens or hundreds of objects that need to be rendered relative to the
 * camera, having to repeatedly write code to render objects relative to the camera for each type
 * of object will quickly become tedious. what happens if you have multiple cameras?
 * remember YAGNI. scalability comes at a cost, if the code is going to be quickly thrown away,
 * like in these demos, then do it the fast way
 */
